    ...
    <div class="mt-8 bg-gray-900 px-4 py-3 sm:rounded-lg border border-gray-800 shadow-sm">
        <form action="{{ route('admin.events.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
            ...
            <div>
                ...
                <select name="type" class="block w-full rounded-md border-0 bg-gray-800 py-1.5 text-white shadow-sm ring-1 ring-inset ring-gray-700 focus:ring-2 focus:ring-inset focus:ring-amber-500 sm:text-sm sm:leading-6">
                ...
                <div class="overflow-hidden shadow ring-1 ring-white/10 sm:rounded-lg">
                    <table class="min-w-full divide-y divide-gray-800">
                        <thead class="bg-gray-900">
                            <tr>
                                <th scope="col" class="py-2 pl-4 pr-3 text-left text-xs font-semibold text-white sm:pl-6">Fecha del Evento</th>
                                <th scope="col" class="px-3 py-2 text-left text-xs font-semibold text-white">Nombre</th>
                                <th scope="col" class="px-3 py-2 text-left text-xs font-semibold text-white">Tipo</th>
                                <th scope="col" class="px-3 py-2 text-left text-xs font-semibold text-white">Estado</th>
                                <th scope="col" class="px-3 py-2 text-left text-xs font-semibold text-white">Asistencia</th>
                        ...
                        <tbody class="divide-y divide-gray-800 bg-gray-950">
                            @forelse ($events as $event)
                                <tr>
                                    <td class="whitespace-nowrap py-1.5 pl-4 pr-3 text-sm font-medium text-white sm:pl-6">
                                        {{ \Carbon\Carbon::parse($event->event_date)->format('d/m/Y H:i') }}
                                    </td>
                                    <td class="whitespace-nowrap px-3 py-1.5 text-sm text-gray-400">
                                        {{ $event->name }}
                                    </td>
                                    <td class="whitespace-nowrap px-3 py-1.5 text-sm text-gray-400 capitalize">
                                        <span class="inline-flex items-center rounded-md bg-{{ $event->color }}-400/10 px-2 py-0.5 text-xs font-medium text-{{ $event->color }}-400 ring-1 ring-inset ring-{{ $event->color }}-400/30">{{ $event->type }}</span>
                                    </td>
                                    <td class="whitespace-nowrap px-3 py-1.5 text-sm text-gray-400">
                                        @if($event->is_active)
                                            <span class="inline-flex items-center rounded-md bg-green-400/10 px-2 py-0.5 text-xs font-medium text-green-400 ring-1 ring-inset ring-green-400/30">Activo</span>
                                        @else
                                            <span class="inline-flex items-center rounded-md bg-gray-400/10 px-2 py-0.5 text-xs font-medium text-gray-400 ring-1 ring-inset ring-gray-400/30">Oculto</span>
                                        @endif
                                    </td>
                                    <td class="whitespace-nowrap px-3 py-1.5 text-sm text-gray-400">
                                        <a href="{{ route('admin.events.attendance', $event) }}" class="inline-flex items-center rounded-md bg-blue-500/10 px-2 py-1 text-xs font-medium text-blue-400 ring-1 ring-inset ring-blue-500/20 hover:bg-blue-500/20">
                                            Pasar Lista
                                        </a>
                                    </td>
                                    <td class="relative whitespace-nowrap py-1.5 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
